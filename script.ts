...
const income_List = document.getElementById('income_List') as HTMLUListElement;
const expense_List = document.getElementById('expense_List') as HTMLUListElement;
const IEInput = document.getElementById('itemDropdown') as HTMLSelectElement;
const itemsPresence = new Set<string>();
let totalIncome = 0;
let totalExpense = 0;
...
function populateDropdownList() {
    const dropdown = document.getElementById('itemDropdown') as HTMLSelectElement;
...
function populateDropdownListDate() {
    const dateDropdown = document.getElementById('Dates') as HTMLSelectElement;
    ...
    if (selectedType === 'income') {
        ...
        ItemValueCAD.addEventListener('keypress', (event) => {
            if (event.key === 'Enter') {
                ...
                const enteredValue = parseFloat((event.target as HTMLInputElement).value);
...
function deleteIncome(event: MouseEvent) {
    const income = (event.target as HTMLElement).parentElement;
    if (income) {
        const IEtext = IEInput.options[IEInput.selectedIndex].text.trim(); // Assuming the value is the type
        const inputElement = income.querySelector('input#incomeTotal') as HTMLInputElement;
...
function deleteExpense(event: MouseEvent) {
    const expense = (event.target as HTMLElement).parentElement;
    if (expense) {
        const IEtext = IEInput.options[IEInput.selectedIndex].text.trim(); // Assuming the value is the type
        const inputElement = expense.querySelector('input#expenseTotal') as HTMLInputElement;
        const deletedValue = parseFloat(inputElement.value);
...
const elemGraph = document.getElementById('graphs') as HTMLCanvasElement;
...
function updateGraph() {
    ...
    income_List.querySelectorAll('li').forEach((li) => {
        ...
        let inputElement = li.querySelector('input[type="string"]') as HTMLInputElement;
...
function loadElementLocaly() {
    ...
    if (listIncomes && listExpenses) {
        ...
        if (Array.isArray(arrayIncomes)) {
            arrayIncomes.forEach((tasktext: string) => {
        ...
        if (Array.isArray(arrayExpenses)) {
            arrayExpenses.forEach((tasktext: string) => {
                const li = document.createElement('li');
                li.textContent = tasktext;
